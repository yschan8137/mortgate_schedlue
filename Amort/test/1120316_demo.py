from dash import Dash, html, callback, callback_context, Input, Output, State, ALL, MATCH
...
class MarkdownWithColorAIO(html.Div):
    ...
    class ids:
        def dropdown(aio_id): return {
            'component': 'MarkdownWithColorAIO',
            'subcomponent': 'dropdown',
            'aio_id': aio_id
        }

        def markdown(aio_id): return {
            'component': 'MarkdownWithColorAIO',
    ...
    def __init__(
        ...
        if 'options' not in dropdown_props:
            dropdown_props['options'] = [
                {'label': i, 'value': i} for i in colors]

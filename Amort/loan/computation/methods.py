...
def _EPP_arr_(
    ...
    def _unique_sum_(t, prepay_amount=prepay_amount, prepay_time=prepay_time): return sum(
        [*np.unique(prepay_amount[:prepay_time[t]])])
...
def _ETP_arr_(
    ...
    for t in range(0, len(_interest_arr_)):
        if t > 0:
            if (prepay_time[t] == 0):
                _principal_payment_.append(
                    loan_amount * principal_ratio_at_(timing=t, prepay_time=prepay_time[t]))
            else:
                if (t == prepay_time[t] and prepay_amount[t] > 0):
                    # 提前支付金額低於前一期餘額
                    if prepay_amount[t] < _residual_[-1]:
                        ...
                        if prepay_amount[t] < _residual_[prepay_time[t-1]] * principal_ratio_at_(timing=t, prepay_time=prepay_time[t-1]):
                            _principal_payment_.append(_residual_[
                                                       prepay_time[t-1]] * principal_ratio_at_(timing=t, prepay_time=prepay_time[t-1]))
                            # 因調整為原始分配時點
                            prepay_time = [
                                prepay_time[t-1] if t_1 == prepay_time[t] else t_1 for t_1 in prepay_time]
                        else:
                        ...
                        _principal_payment_.append(
                            (loan_amount - sum(_principal_payment_[:t])))
                else:
                    _principal_payment_.append(_residual_[
                                               prepay_time[t]] * principal_ratio_at_(timing=t, prepay_time=prepay_time[t]))

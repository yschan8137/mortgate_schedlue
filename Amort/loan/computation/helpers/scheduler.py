      ...
      if false_kwargs:
        for false in false_kwargs:
          if isinstance(false, str):
            raise KeyError(f"Error in arguments: {false}.  The available arguments: {kws_spec}")
          elif isinstance(false, dict):
            raise KeyError(f"Error in arguments: {[*false.values()]} in {[*false.keys()]}.  The available arguments: {kws_spec}")
          else:
            raise KeyError(f"Error in arguments types: {false[1]} in {false[0]}.  The right type is: {kws_spec[[*false[0].keys()][0]][[*[*false[0].values()][0]][0]]}")

def scheduler(
    loan_period: int, 
    convert_to_month_rate = True, 
    **kwargs: dict) -> list:
  """
  ...
  4. prepay_arr 
   ...
   4.3 accumulator(False by defaulit): accumulate the amount at the change pointa.
  
  Example:
  A 10 yrs multistage interest rate loan of 500,000 with interest rate chane at t= 12, and prepay 200,000 at t= 40 and prepay 400,000 t= 60
  None that as the prepayment will exceed the total loan amount, thus the balance will be 0 after t = 60.
  scheduler(
    loan_period=10, 
    interest_arr= {'interest': [1.38, 1.01],
            'multi_arr': [12]
            }, 
     loan= 500_000,
     prepay_arr = {'amount': [200_000, 400_000],
             'multi_arr': [40, 60]
            }
     )  
  """
  ...
  interest_arr = [0] + sorted(ensure_list_type(kwargs.get('interest_arr', {}).get('multi_arr', [])))
  loan = ensure_list_type(kwargs.get('loan', 0))
  prepay = [0] + ensure_list_type(kwargs.get('prepay_arr', {}).get('amount', []))
  prepay_arr = [0] + sorted(ensure_list_type(kwargs.get('prepay_arr', {}).get('multi_arr', [])))
  ...
  if len(interest) > 1:
    if len(interest_arr) == 0:
      raise KeyError('\r' + 'The [multi_arr] should be specified under multistage interest rate frsmework' + '\n')
    if len(interest_arr) > len(interest):
      raise ValueError('\r' + '[time] should be indicated as the rate adjusting points. The example is shown in arguments discription for reference' + '\n')
    elif len(interest_arr) < len(interest):
      print(len(interest_arr), len(interest))
      raise ValueError('\r' + f'The adjustments of the interest rate is {len(interest)}, exceed the given time points, which is {len(interest_arr)}' + '\n') 
  ...
  def _arr_(
    ...
    #  if len(amount_arr) != len(time_arr):
      # sys.stdout.write('\r' + f'Note: The length of [amount_arr] is incosistent with that of [time_arr] which is {len(time_arr)}, otherwider the output would become a sequent list of [time_arr].')
      # sys.stdout.flush()
  ...
  res = map(
        lambda loan, interest, prepay: (
            loan - (prepay if loan > 0 else (loan - 1 if prepay == 0 and interest > 0 else -prepay))
            ) * (interest if interest > 0 else 1), 
            _arr_(amount_arr= loan), 
            map(lambda x: (x/12 if convert_to_month_rate == True else x), _arr_(amount_arr= interest, time_arr = interest_arr)), 
